...
class CCLSTM_cell(Module):
    def __init__(self,
        ...
        super(CCLSTM_cell, self).__init__()
...
class CCLSTM(Module):
    ...
    def forward(self,
        ...
        for t in range(1, self.enc_len + self.fore_len - 1):
            if t < self.enc_len:
                inputs = torch.cat((x[:, t], spa_vars), 1)
            else:
                mask_t = mask[t - self.enc_len]
